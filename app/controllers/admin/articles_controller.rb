...
#  before_filter :login_required
  ...
  # 検索 by 場所、オーナー、分類１
  def search
    @articles = Article.search(params[:q1], params[:q2]).
      paginate(page: params[:page], per_page: 20)
    render "index"
  end
...
#    @article = Article.new(params[:article])
    ...
    if @article.save
      redirect_to [:admin,@article],notice: "サーバリソース情報を登録しました。"
    else
      render "new"
    end
  end
...
#    @article.assign_attributes(params[:article])
    ...
    if @article.save
      redirect_to [:admin,@article],notice: "サーバリソース情報を更新しました。"
    else
      render "edit"
    end
  end
  ...
  def destroy
    @article = Article.find(params[:id])
    @article.destroy
    redirect_to :admin_articles, notice: "サーバリソース情報を削除しました。"
  end
  def download
    @articles = Article.order(":asciino, category1, category2")
